...
import static a4.svpino.artifacts.Hazard.HazardType;

import a4.svpino.artifacts.Hazard;
import a4.svpino.artifacts.PlannerFactory;
			...
			case ValueIteration:
				runAlgorithm(analysis, problem, domain, hashingFactory, initialState, new PlannerFactory() {
					...
					public Planner createPlanner(int episodeIndex, SADomain domain, HashableStateFactory hashingFactory, SimulatedEnvironment simulatedEnvironment) {
						int maxIter = 1000;
//						maxIter = episodeIndex;
						return new ValueIteration(domain, 0.99, hashingFactory, 0.001, 1000);
					}
				}, algorithm);
				break;
			...
			default:
				runAlgorithm(analysis, problem, domain, hashingFactory, initialState, new PlannerFactory() {
					...
					@Override
					public Planner createPlanner(int episodeIndex, SADomain domain, HashableStateFactory hashingFactory, SimulatedEnvironment simulatedEnvironment) {
						EpsilonGreedy  eGreedy = new EpsilonGreedy(0.5 );
						QLearning agent = new QLearning(domain, 0.99, hashingFactory, 0.3, 0.1, 1000);
	...
	private static void learningExperimenter(Problem problem, LearningAgent agent, SimulatedEnvironment simulatedEnvironment) {
		LearningAlgorithmExperimenter experimenter = new LearningAlgorithmExperimenter(simulatedEnvironment, 10, 500, new LearningAgentFactory() {
		...
		experimenter.writeEpisodeDataToCSV("output/q-results.csv");
	}
